"""
app/main.py
Integra login / profile selection page.
Profile cards replace the selectbox. No nav bar on this page.
"""
...
from app.styles import inject_css, inject_bottom_nav, disclaimer_chip
...
st.markdown(
    ...
    .integra-profile-card:hover {{
        border-color: #c4956a;
        background: rgba(196,149,106,0.04);
    }}
    .integra-profile-avatar {{
        width: 42px;
        height: 42px;
        border-radius: 13px;
        display: flex;
    ...
    .integra-profile-avatar.jordan {{
        background: linear-gradient(135deg, rgba(196,149,106,0.15), rgba(123,158,166,0.1));
        border: 1px solid rgba(196,149,106,0.3);
        color: #c4956a;
    }}
    .integra-profile-avatar.alex {{
        background: linear-gradient(135deg, rgba(123,166,143,0.15), rgba(166,196,123,0.1));
        border: 1px solid rgba(123,166,143,0.3);
        color: #7ba68f;
    }}
    .integra-profile-info {{ flex: 1; min-width: 0; }}
    .integra-profile-name {{
        font-family: 'DM Sans', sans-serif;
        font-size: 15px;
        font-weight: 500;
        color: #e8e3db;
    ...
    .badge-new {{
        background: rgba(123,166,143,0.12);
        color: #7ba68f;
        border: 1px solid rgba(123,166,143,0.22);
    }}
    </style>
    """,
    unsafe_allow_html=True,
...
st.markdown(
    ...
    f'margin-bottom:32px;margin-top:16px;">'
    f'<div style="width:56px;height:56px;margin-bottom:14px;">{LOGO_SVG}</div>'
    f'<div style="font-family:\'DM Serif Display\',Georgia,serif;font-size:32px;'
    f'color:#e8e3db;letter-spacing:-0.01em;margin-bottom:6px;">Integra</div>'
    f'<div style="font-size:13px;font-weight:300;color:#5a5652;text-align:center;'
    f'line-height:1.5;">Your integration companion.<br>Choose a profile to begin.</div>'
    f'</div>',
    unsafe_allow_html=True,
)

# ---------------------------------------------------------------------------
# Profile cards (rendered as HTML; selection via Streamlit buttons below)
# ---------------------------------------------------------------------------

st.markdown(
    '<div class="integra-profile-card">'
    '<div class="integra-profile-avatar jordan">J</div>'
    '<div class="integra-profile-info">'
    '<div class="integra-profile-name">Jordan</div>'
    '<div class="integra-profile-desc">11 entries across 6 weeks</div>'
    '</div>'
    '<div class="integra-profile-badge badge-established">Established</div>'
    '</div>',
    unsafe_allow_html=True,
)

st.markdown('<div class="primary-btn" style="margin-bottom:10px;">', unsafe_allow_html=True)
if st.button("Continue as Jordan", use_container_width=True, key="btn_jordan"):
    if st.session_state["user"] != "Jordan":
    ...
    st.session_state["user"] = "Jordan"
    st.switch_page("pages/0_Home.py")
st.markdown('</div>', unsafe_allow_html=True)

st.markdown(
    '<div class="integra-profile-card">'
    '<div class="integra-profile-avatar alex">A</div>'
    '<div class="integra-profile-info">'
    '<div class="integra-profile-name">Alex</div>'
    '<div class="integra-profile-desc">New user, no entries yet</div>'
    '</div>'
    '<div class="integra-profile-badge badge-new">New</div>'
    '</div>',
    unsafe_allow_html=True,
)

if st.button("Continue as Alex", use_container_width=True, key="btn_alex"):
...
st.markdown(
    '<p style="font-size:13px;font-weight:300;color:#5a5652;margin-bottom:10px;">'
    'Just want to talk? Chat with Indy without a profile.</p>',
    unsafe_allow_html=True,
)

if st.button("Chat with Indy as a guest", use_container_width=True, key="btn_guest"):
